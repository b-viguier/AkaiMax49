...
namespace {
  ...
  byte triggerBlockButtons(ButtonBlockAddr blockAddr, byte &state, ButtonManager::Callback callback1, ButtonManager::Callback callback2,
...
void ButtonManager::update() {
  ...
  _states[0] = triggerBlockButtons(U524, _states[0], _callbacks[TIME_4], _callbacks[TIME_4T],
                                   _callbacks[TIME_8], _callbacks[TIME_8T],
                                   _callbacks[TIME_16], _callbacks[TIME_16T],
                                   _callbacks[TIME_32], _callbacks[TIME_32T]
  );

  _states[1] = triggerBlockButtons(U221, _states[1], _callbacks[BANK_1_8],
  ...
  _states[2] = triggerBlockButtons(U220, _states[2], _callbacks[UP], _callbacks[DOWN],
                                   _callbacks[LEFT], _callbacks[RIGHT],
                                   _callbacks[EDIT_STORE], _callbacks[PROGRAM_STD],
                                   _callbacks[GLOBAL_HUI], _callbacks[PROG_CHANGE]
  );
  _states[3] = triggerBlockButtons(U223, _states[3], _callbacks[PREV], _callbacks[STOP],
                                   _callbacks[PLAY], _callbacks[RECORD], _callbacks[NEXT], _callbacks[SHIFT],
                                   _callbacks[TEMPO], _callbacks[ENTER]
  );
  _states[4] = triggerBlockButtons(U222, _states[4], _callbacks[OCT_DOWN], _callbacks[OCT_UP],
                                   _callbacks[PLAY], _callbacks[RECORD], _callbacks[NEXT], _callbacks[SHIFT],
                                   _callbacks[ARP_MODE], _callbacks[SEQ_SELECT]
  );
  _states[5] = triggerBlockButtons(U272, _states[5], _callbacks[ON_OFF], _callbacks[LATCH],
